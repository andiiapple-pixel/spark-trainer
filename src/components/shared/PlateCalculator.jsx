...
const PLATE_COLOR_MAP = {
  ...
  5: '#888888', 2.5: '#555555', 1.25: '#E8FF00',
  45: '#ef4444', 35: '#3b82f6', 'other': '#555555'
};
...
export default function PlateCalculator({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.8)' }}>
      <div className="w-full max-w-[430px] p-5 pb-8 max-h-[85vh] overflow-y-auto"
        style={{ background: '#111111', border: '1px solid #222222', borderRadius: 0 }}>
        <div className="flex items-center justify-between mb-4">
          <h2 style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 18, color: '#FFFFFF', textTransform: 'uppercase' }}>Calculator</h2>
          <button onClick={onClose} className="btn-press p-1"><X size={20} style={{ color: '#555555' }} /></button>
        </div>

        {/* Tabs */}
        <div className="flex mb-4" style={{ borderBottom: '1px solid #222222' }}>
          {['plates', 'warmup'].map(t => (
            <button key={t} onClick={() => setTab(t)}
              className="flex-1 py-2.5 text-sm font-semibold relative btn-press"
              style={{ color: tab === t ? '#FFFFFF' : '#555555' }}>
              {t === 'plates' ? 'Plate Calculator' : 'Warm-up Calculator'}
              {tab === t && (
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-12 h-0.5" style={{ background: '#E8FF00', borderRadius: 0 }} />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2 mb-4">
          {['kg', 'lbs'].map(u => (
            <button key={u} onClick={() => switchUnit(u)}
              ...
              style={{
                background: unit === u ? '#E8FF00' : '#111111',
                color: unit === u ? '#000000' : '#888888',
                border: `1px solid ${unit === u ? '#E8FF00' : '#222222'}`,
                borderRadius: 0,
              }}>
              {u}
            </button>
        ...
        {/* Target weight input */}
        <div className="mb-3">
          <label className="text-xs font-semibold mb-1 block" style={{ color: '#555555', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
            {tab === 'plates' ? 'TARGET WEIGHT' : 'WORKING WEIGHT'}
          </label>
          <div className="flex items-center gap-2 px-3 py-3" style={{ background: '#111111', border: '1px solid #222222', borderRadius: 0 }}>
            <input
              type="number"
        ...
        {tab === 'plates' && (
          <>
            <div className="mb-4">
              <label className="text-xs font-semibold mb-1 block" style={{ color: '#555555', letterSpacing: '0.05em', textTransform: 'uppercase' }}>BARBELL WEIGHT</label>
              <div className="flex gap-2">
                {(unit === 'kg' ? ['15', '20', 'custom'] : ['35', '45', 'custom']).map(w => (
                  <button key={w} onClick={() => w !== 'custom' && setBarbellWeight(w)}
                    className="flex-1 py-2 text-sm btn-press"
                    style={{
                      background: barbellWeight === w ? '#E8FF00' : '#111111',
                      color: barbellWeight === w ? '#000000' : '#888888',
                      border: `1px solid ${barbellWeight === w ? '#E8FF00' : '#222222'}`,
                      borderRadius: 0,
                    }}>
                    {w === 'custom' ? (
            ...
            {target > 0 && (
              <div className="p-4" style={{ background: '#0A0A0A', border: '1px solid #222222', borderRadius: 0 }}>
                <p className="text-xs font-semibold mb-3 text-center" style={{ color: '#555555', letterSpacing: '0.06em', textTransform: 'uppercase' }}>EACH SIDE</p>
                {plates.length === 0 ? (
                    ...
                    {plates.map(p => (
                      <div key={p.weight} className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          <div className="w-3 h-3" style={{ background: PLATE_COLOR_MAP[p.weight] || '#555555', borderRadius: 0 }} />
                          <span style={{ color: '#FFFFFF' }}>{p.weight}{unit}</span>
                        </div>
                        <span style={{ color: '#888888' }}>× {p.count} per side</span>
                      </div>
                    ))}
                    <div className="mt-2 pt-2" style={{ borderTop: '1px solid #222222' }}>
        ...
        {tab === 'warmup' && (
          <>
            <div className="mb-4">
              <label className="text-xs font-semibold mb-1 block" style={{ color: '#555555', letterSpacing: '0.05em', textTransform: 'uppercase' }}>WARM-UP SETS</label>
              <div className="flex gap-2">
                {[2, 3, 4, 5].map(n => (
                  <button key={n} onClick={() => setWarmupSets(n)}
                    className="flex-1 py-2 text-sm font-medium btn-press"
                    style={{
                      background: warmupSets === n ? '#E8FF00' : '#111111',
                      color: warmupSets === n ? '#000000' : '#888888',
                      border: `1px solid ${warmupSets === n ? '#E8FF00' : '#222222'}`,
                      borderRadius: 0,
                    }}>
                    {n}
                  </button>
            ...
            {target > 0 && (
              <div className="space-y-2">
                {warmups.map((set, i) => (
                  <div key={i} className="flex items-center gap-3 p-3" style={{ background: '#0A0A0A', border: '1px solid #222222', borderRadius: 0 }}>
                    <div className="w-8 h-8 flex items-center justify-center text-xs font-bold"
                      style={{ background: '#111111', color: '#888888', borderRadius: 0 }}>
                      {i + 1}
                    </div>
                ...
                <div className="flex items-center gap-3 p-3" style={{ background: '#111111', border: '1px solid #E8FF00', borderRadius: 0 }}>
                  <div className="w-8 h-8 flex items-center justify-center text-xs font-bold"
                    style={{ background: '#E8FF00', color: '#000000', borderRadius: 0 }}>W</div>
                  <span className="font-semibold text-sm tabular-nums" style={{ color: '#E8FF00' }}>{target}{unit} — Working sets</span>
                </div>
              </div>
            )}
          </>
...
function PlateVisual({ weight, unit }) {
  const color = PLATE_COLOR_MAP[weight] || '#555555';
  const height = Math.max(24, Math.min(60, weight * 2));
  return (
    <div className="flex flex-col items-center justify-center text-xs font-bold"
      style={{ background: color, width: 18, height, color: '#fff', fontSize: 9, writingMode: 'vertical-rl', borderRadius: 0 }}>
      {weight}
    </div>
  );
}
